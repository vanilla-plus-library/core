...
# Success
execute if score #load vplib.load matches 1 run function vplib:core/__ver/load/load

# Failed

## Messages
execute if score #load vplib.load matches 0 run tellraw @a [{"text":"[Debug]: ","color":"red","bold":true},{"text":"Vanilla+ Library: Core failed to load. It requires Minecraft 1.15+.","color":"white","bold":false}]

## Reset vplib.loaded version
